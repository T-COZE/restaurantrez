...
function openSeat(req, res, next) {
...
async function tableExists(req, res, next) {
  ...
  next({
    status: 404,
    message: `Table ${table_id} Not Found or does not exist.`,
  });
}
...
function validTable(req, res, next) {
  ...
  if (table.capacity < reservation.people) {
    return next({
      status: 400,
      message:
        "Table does not have enough capacity for this reservation",
    });
  }
  if (table.reservation_id !== null) {
    return next({
...
module.exports = {
  ...
  create: [hasValidProps, asyncErrorBoundary(create)],
